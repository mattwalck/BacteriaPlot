function [header, avgPeak, fitPeak, lambdaPeak, fitResult, lambda, data] = main(folder, suffix, set, startLambda, endLambda, wSize, nPoints)
...
lambdaPeak = zeros(1,nExp); fitPeak = zeros(1,nExp); avgPeak = zeros(1,nExp);

% Start fitting experiments
for i=1:nExp
    spectrum = data(:,i); % Experimental data
    fitResult{i} = createFit(lambda, spectrum); % Fit to gaussian
    coeff = coeffvalues(fitResult{i}); lambdaPeak(i) = coeff(2); % Peak wavelength
    fit = feval(fitResult{i},lambda); % Gaussian data
    ci = find(lambda == round(lambdaPeak(i))); % Find location of peak in array
    if isempty(ci)
